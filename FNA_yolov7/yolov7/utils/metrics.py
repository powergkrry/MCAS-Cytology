...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.2): # modified from iou_thres=0.45
...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    # there is only one class so do not need to plot
    ...
    ax.plot(px, py.mean(1), linewidth=3, color='blue', label='all classes %.3f mAP@0.05' % ap[:, 0].mean())
    ax.set_xlabel('Recall', fontsize=18)
    ax.set_ylabel('Precision', fontsize=18)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.tick_params(axis='both', which='major', labelsize=16)
    plt.legend(loc="upper right", fontsize=18) # bbox_to_anchor=(1.04, 1), 
